...
class UserDataRepository(private val userDao: UserDao) {
    ...
    val unlockedSeeds: Flow<Set<SeedType>> = userDao.getUnlockedSeeds()
        .map { list -> list.mapNotNull { SeedType.fromId(it.seedId) }.toSet() }
    ...
    fun getGrownTreesStream(): Flow<List<GrownTree>> = callbackFlow {
        ...
        val listener = collection.orderBy("plantedAt", Query.Direction.DESCENDING)
    ...
    suspend fun addPoints(amount: Int) {
        withContext(Dispatchers.IO) { userDao.increasePoints(amount); syncUserProfileToCloud() }
    }

    suspend fun deductPoints(amount: Int) {
        withContext(Dispatchers.IO) { userDao.decreasePoints(amount); syncUserProfileToCloud() }
    }
    ...
    private suspend fun syncUserProfileToCloud() {
        ...
        try {
            ...
            firestore.collection("users").document(uid)
                .set(hashMapOf("points" to points, "unlockedSeeds" to seeds), SetOptions.merge())
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (document.exists()) {
                    ...
                    val seedsList = (document.get("unlockedSeeds") as? List<*>)
    ...
    fun getTasksStream(dateMs: Long): Flow<List<TodoTask>> = callbackFlow {
        ...
        val listener = collection.whereGreaterThanOrEqualTo("date", startOfDay)
            .whereLessThanOrEqualTo("date", endOfDay).addSnapshotListener { snapshot, _ ->
            if (snapshot != null) {
                val tasks = snapshot.documents.mapNotNull { doc ->
                    doc.toObject(TodoTask::class.java)?.copy(id = doc.id)
                }
                val sortedTasks =
                    tasks.sortedWith(compareByDescending<TodoTask> { it.isPinned }.thenBy { it.isCompleted }
    ...
    suspend fun updateTaskInCloud(task: TodoTask) {
        val uid = auth.currentUser?.uid ?: return; if (task.id.isEmpty()) return
        try {
            firestore.collection("users").document(uid).collection("tasks").document(task.id)
                .set(task).await()
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    private fun getStartOfDay(time: Long): Long {
        ...
        cal.set(java.util.Calendar.HOUR_OF_DAY, 0); cal.set(java.util.Calendar.MINUTE, 0); cal.set(
    ...
    private fun getEndOfDay(time: Long): Long {
        val cal = java.util.Calendar.getInstance().apply { timeInMillis = time }
        cal.set(java.util.Calendar.HOUR_OF_DAY, 23); cal.set(
    ...
    suspend fun getTaskStats(): TaskStats {
        ...
        try {
            val snapshot =
                firestore.collection("users").document(uid).collection("tasks").get().await()
    ...
    suspend fun checkAndSyncAchievements(): List<String> {
        ...
        try {
            ...
            val currentUnlocked =
                (userDoc.get("unlockedAchievements") as? List<*>)
                    ?.filterIsInstance<String>()
                    ?.toMutableSet()
                    ?: mutableSetOf()
    ...
    suspend fun selectAchievement(achievementId: String) {
        ...
        try {
            firestore.collection("users").document(uid)
                .update("selectedAchievementId", achievementId).await()
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    suspend fun clearLocalData() {
        withContext(Dispatchers.IO) {
            ...
            val defaultProfile = UserProfile(id = 1, points = 0); userDao.upsertUserProfile(
            defaultProfile
        )
            val defaultSeed = UnlockedSeed(SeedType.NORMAL.id); userDao.insertUnlockedSeed(
            defaultSeed
        )
        }
    }
    ...
    suspend fun deleteAccountData() {
        withContext(Dispatchers.IO) {
            try {
                val userDoc = firestore.collection("users").document(
                    auth.currentUser?.uid ?: return@withContext
                ); userDoc.collection("garden").get().await()
                    .forEach { it.reference.delete() }; userDoc.collection("tasks").get().await()
                    .forEach { it.reference.delete() }; userDoc.delete().await(); clearLocalData()
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
    ...
    suspend fun toggleTaskStatus(taskId: String, currentStatus: Boolean) {
        val uid = auth.currentUser?.uid ?: return; try {
            firestore.collection("users").document(uid).collection("tasks").document(taskId)
                .update("completed", !currentStatus).await()
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
